...
Kullanım:
 - python kralzeka_app.py  # yerel test (env FLASK_RUN_LOCAL=1)
 - deploy: gunicorn kralzeka_app:app
ENV:
 - GROQ_API_KEY (opsiyonel)
 - HF_API_KEY   (opsiyonel)
 - SECRET_KEY   (opsiyonel; yoksa rastgele üretilir)
 - KRALZEKA_DB  (opsiyonel; default ./kralzeka.db)
Not: .env sadece KEY'leri içermeli (istemediğin değerleri kod içine koymadım).
"""
...
IMAGE_DAILY_LIMIT = 5
ADMIN_USERNAME = "enes"
ADMIN_PASSWORD = "enes1357924680"  # ilk admin (kodda korunacak)
...
# ---------- AI BACKENDS (basit, güvenli çağrı) ----------
...
def call_groq_chat(prompt: str, model=DEFAULT_GROQ_MODEL):
    ...
    try:
        ...
        logger.exception("Groq çağrısı başarısız")
        raise
...
# ---------- HTML ŞABLONLARI (her route için kendi tam HTML'i) ----------

BASE_STYLE = """
<style>
  :root{--bg:#071018;--card:#071a1a;--muted:#9cb8b8;--accent:#2f7f7f;--text:#d8efe9}
  body{background:var(--bg);color:var(--text);font-family:Inter,Segoe UI,Arial,Helvetica,sans-serif;margin:0;padding:20px}
  .wrap{max-width:1000px;margin:0 auto}
  header{display:flex;justify-content:space-between;align-items:center}
  h1{margin:0;font-size:28px}
  .card{background:linear-gradient(180deg, rgba(0,0,0,0.45), rgba(0,0,0,0.25));padding:18px;border-radius:12px;box-shadow:0 10px 30px rgba(0,0,0,0.6);margin-top:18px}
  input,textarea,button,select{font-size:15px;padding:10px;border-radius:8px;border:1px solid rgba(255,255,255,0.04);background:#0b1517;color:var(--text);width:100%;box-sizing:border-box}
...
<head>
  <meta charset="utf-8">
  <title>Giriş - KralZeka</title>
  {BASE_STYLE}
</head>
<body>
  <div class="wrap">
    <header><h1>KralZeka v1</h1></header>
    <div class="card">
      <h3>Giriş</h3>
...
<body>
  <div class="wrap">
    ...
    <div class="card">
      ...
      <div class="small muted">İlk admin ({{{{ admin_username }}}}) korunur.</div>
    </div>
    <footer>© KralZeka v1</footer>
  </div>
</body>
...
<head>
  <meta charset="utf-8">
  <title>Admin Panel - KralZeka</title>
  {BASE_STYLE}
</head>
<body>
  <div class="wrap">
    <header>
      <h1>Admin Panel</h1>
      <div>Merhaba, <strong>{{{{ user['username'] }}}}</strong> | <a href="{{{{ url_for('index') }}}}">Siteye Dön</a></div>
    </header>
    ...
    <div class="card">
      <h3>Kod Yazma / Öğretme (Admin only)</h3>
      <div class="small">Buraya kod talebinizi yazın; AI önce sorular sorabilir, sonra kod ve açıklama üretir.</div>
      <form method="post" action="{{{{ url_for('admin_code_tool') }}}}">
        <textarea name="prompt" rows="4" placeholder="Örnek: 'Flask ile basit bir REST API örneği yaz ve adım adım açıkla'..." required></textarea>
        <div style="margin-top:8px"><button>Talep Gönder</button></div>
      </form>
      <div class="small muted">Üretilen sonuçlar loglanır, kayıt altına alınır.</div>
    </div>

    <div class="card">
      <h3>Loglar (son 100)</h3>
      {{% for l in logs %}}
...
<!doctype html>
<html lang="tr">
<head><meta charset="utf-8"><title>Hata</title>{BASE_STYLE}</head>
<body>
  <div class="wrap">
    <div class="card"><h3>Sunucu Hatası (500)</h3><p>Bir hata oluştu. Admin'e bildirin veya logları kontrol edin.</p></div>
    <footer>© KralZeka v1</footer>
  </div>
</body>
</html>
...
def index():
    ...
    msgs = cur.fetchall()
    return render_template_string(INDEX_HTML, user=user, messages=msgs, image_limit=IMAGE_DAILY_LIMIT)
...
def register():
    if request.method == "POST":
        ...
        if not username or not password:
            return render_template_string(REGISTER_HTML, error="Alanları doldurun", admin_username=ADMIN_USERNAME)
        if password != password2:
            return render_template_string(REGISTER_HTML, error="Parolalar eşleşmiyor", admin_username=ADMIN_USERNAME)
        if get_user_by_username(username):
            return render_template_string(REGISTER_HTML, error="Kullanıcı adı alınmış", admin_username=ADMIN_USERNAME)
        uid = create_user(username, password, 0)
        if not uid:
            return render_template_string(REGISTER_HTML, error="Kayıt başarısız", admin_username=ADMIN_USERNAME)
        log_event("INFO", f"Kayıt: {username}")
        session["user_id"] = uid
        return redirect(url_for("index"))
    return render_template_string(REGISTER_HTML, error=None, admin_username=ADMIN_USERNAME)
...
def chat():
    ...
    db.commit()
    try:
        # basit: öncelik Groq, fallback HF
...
def admin_panel():
    ...
    logs = cur.fetchall()
    return render_template_string(ADMIN_HTML, users=users, logs=logs, admin_username=ADMIN_USERNAME, user=g.user)
...
def admin_code_tool():
    ...
    if not prompt:
        return redirect(url_for("admin_panel"))
    try:
        # admin kod öğretme: ai_chat çağrısı
        ai_response = ai_chat(f"Admin kod-öğretme asistanı. İsteği: {prompt}\nLütfen önce hangi dil/çerçeve kullanılacağını sor, sonra adım adım açıkla ve örnek kod ver.")
    except Exception as e:
...
def api_generate_image():
    ...
    try:
        ...
        # store truncated preview in messages
        db = get_db()
        cur = db.cursor()
        preview = f"data:image/png;base64,{b64[:180]}..."
        cur.execute("INSERT INTO messages (user_id, sender, content) VALUES (?, ?, ?)", (user["id"], "KralZeka (görsel)", preview))
        db.commit()
